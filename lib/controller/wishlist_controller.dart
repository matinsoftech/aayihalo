...
//nirajan new function add to wish list
  void newAddToWishList(int productsId, {bool getXSnackBar = false}) async {
    Response response = await wishListRepo.newAddWishList(productsId);
    if (response.statusCode == 200) {
      // if(isStore) {
      //   _wishStoreIdList.forEach((storeId) {
      //     if(storeId == store.id){
    ...
    if (response.statusCode == 200) {
      ...
      if (response.body['item'] != null) {
        response.body['item'].forEach((item) async {
          if (item['module_type'] == null || !Get.find<SplashController>().getModuleConfig(item['module_type']).newVariation! || item['variations'] == null || item['variations'].isEmpty || (item['food_variations'] != null && item['food_variations'].isNotEmpty)) {
