...
class TrackDetailsView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    //   ) / 1000;
    // }
    if (track.deliveryAddress != null && track.deliveryAddress!.latitude != null && track.deliveryAddress!.longitude != null && track.deliveryMan != null && track.deliveryMan!.lat != null && track.deliveryMan!.lng != null) {
      // Parse the latitude and longitude values safely
      double? addressLatitude = double.tryParse(track.deliveryAddress!.latitude!);
      double? addressLongitude = double.tryParse(track.deliveryAddress!.longitude!);
      double? deliveryManLatitude = double.tryParse(track.deliveryMan!.lat!);
      double? deliveryManLongitude = double.tryParse(track.deliveryMan!.lng!);

      // Ensure parsing was successful before using the values
      if (addressLatitude != null && addressLongitude != null && deliveryManLatitude != null && deliveryManLongitude != null) {
        distance = Geolocator.distanceBetween(
              addressLatitude,
              addressLongitude,
              deliveryManLatitude,
        ...
        print("Error: One or more coordinates could not be parsed.");
      }
    } else {
              ...
              children: [
                ...
                Row(
                  children: [
                    ClipOval(
                        child: CustomImage(
                      image: '${takeAway ? Get.find<SplashController>().configModel!.baseUrls!.storeImageUrl : Get.find<SplashController>().configModel!.baseUrls!.deliveryManImageUrl}/${takeAway ? track.store != null ? track.store!.logo : '' : track.deliveryMan!.image}',
                      height: 35,
                      width: 35,
                      fit: BoxFit.cover,
                    )),
