...
class AddAddressScreen extends StatefulWidget {
  ...
  const AddAddressScreen({Key? key, this.newaddress, this.addressid, this.couponCode, required this.fromCheckout, required this.fromRide, this.address, this.zoneId, this.forGuest = false, this.fromNavBar = false}) : super(key: key);
...
class _AddAddressScreenState extends State<AddAddressScreen> {
  ...
  String? _countryDialCode = Get.find<AuthController>().getUserCountryCode().isNotEmpty ? Get.find<AuthController>().getUserCountryCode() : CountryCode.fromCountryCode(Get.find<SplashController>().configModel!.country!).dialCode;
                                        ...
                                        Container(
                                          ...
                                          child: ClipRRect(
                                            ...
                                            child: Stack(clipBehavior: Clip.none, children: [
                                              ...
                                              Center(child: !locationController.loading ? Image.asset(Images.pickMarker, height: 50, width: 50) : const CircularProgressIndicator()),
                                              Positioned(
                                                bottom: 10,
                                                right: 0,
                                                child: InkWell(
                                                    ...
                                                    if (ResponsiveHelper.isDesktop(Get.context)) {
                                                      showGeneralDialog(
                                                          ...
                                                          pageBuilder: (_, __, ___) {
                                                            return SizedBox(height: 300, width: 300, child: PickMapScreen(fromSignUp: false, canRoute: false, fromAddAddress: true, route: null, googleMapController: locationController.mapController));
                                                          });
                                                    } else {
                                                      Get.toNamed(
                                                        RouteHelper.getPickMapRoute('add-address', false),
