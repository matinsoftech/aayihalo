...
class _AddAddressScreenState extends State<AddAddressScreen> {
  ...
  Future<void> storeIdGet() async {
    await _itemController.getHomeScreenData(
      lat: Get.find<LocationController>().getUserAddress()!.latitude!,
      long: Get.find<LocationController>().getUserAddress()!.longitude!,
    );
    if (_itemController.homeScreenDataModel != null &&
        _itemController.homeScreenDataModel!.nearestStore != null) {
      print("not null...");
      setState(() {
        store_id = _itemController.homeScreenDataModel!.nearestStore!.id;
      });
    } else {
  ...
  void initCall() {
    ...
    if (widget.address == null) {
      ...
      } else if (widget.address!.addressType == 'hotel') {
        Get.find<LocationController>().setAddressTypeIndex(3, isUpdate: false);
      } else {
                ...
                locationController
                  ...
                  if (response.isSuccess) {
                    ...
                    showCustomSnackBar('new_address_added_successfully'.tr,
                        isError: false);
                    locationController.getAddressList().then((_) {
                      if (widget.newaddress == true) {
                        id = locationController.addressList![0].id!;
                        cartController
                            .placeOrder(
